      ...
      <td className="py-4">
        <div>
          ...
          <div className="flex gap-2 items-center">
            <span className={`text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded ${candidate.confidence_level === 'high' ? 'bg-emerald-500/10 text-emerald-400' :
      ...
      <td className="w-64 max-w-[250px]">
        <div className="flex flex-wrap gap-1.5">
          ...
          {candidate.inferred_matches.slice(0, Math.max(0, 3 - candidate.direct_matches.length)).map((m, i) => (
            <span key={`i-${i}`} className="text-xs bg-[rgba(135,163,48,0.1)] border border-[var(--border-accent)] px-2 py-1 rounded text-[var(--accent-indigo)]" title={`Inferred via ${m.matched_via}`}>
